...
gboolean Args::mc_args__force_xterm = FALSE;
gboolean Args::mc_args__nomouse = FALSE;
gboolean Args::mc_args__force_colors = FALSE;
gboolean Args::mc_args__nokeymap = FALSE;
char* Args::mc_args__last_wd_file = nullptr;
char* Args::mc_args__netfs_logfile = nullptr;
char* Args::mc_args__keymap_file = nullptr;

#ifdef ENABLE_VFS_SMB
int Args::mc_args__debug_level = 0;
#endif

void* Args::mc_run_param0 = nullptr;
char* Args::mc_run_param1 = nullptr;



gboolean Args::mc_args__show_version = FALSE;
GOptionContext* Args::context;

#ifdef ENABLE_SUBSHELL
gboolean Args::mc_args__nouse_subshell = FALSE;
#endif /* ENABLE_SUBSHELL */

gboolean Args::mc_args__show_datadirs = FALSE;
gboolean Args::mc_args__show_datadirs_extended = FALSE;

#ifdef ENABLE_CONFIGURE_ARGS
gboolean Args::mc_args__show_configure_opts = FALSE;
#endif

GOptionGroup* Args::main_group;

const GOptionEntry Args::argument_main_table[] = {
    /* *INDENT-OFF* */
    /* generic options */
    {
    ...
    {
        nullptr, '\0', 0, G_OPTION_ARG_NONE, nullptr, nullptr, nullptr /* Complete struct initialization */
    }
    /* *INDENT-ON* */
};

GOptionGroup* Args::terminal_group;

const GOptionEntry Args::argument_terminal_table[] = {
    /* *INDENT-OFF* */
    /* terminal options */
    {
...
GOptionGroup* Args::color_group;

/* #define ARGS_COLOR_OPTIONS G_OPTION_FLAG_IN_MAIN */
const GOptionEntry Args::argument_color_table[] = {
    /* *INDENT-OFF* */
    /* color options */
    {
...
char* Args::mc_args__loc__colors_string = nullptr;
char* Args::mc_args__loc__footer_string = nullptr;
char* Args::mc_args__loc__header_string = nullptr;
char* Args::mc_args__loc__usage_string = nullptr;

void Args::mc_setup_run_mode(char** argv)
{
...
gboolean Args::mc_args_parse(int* argc, char*** argv, const char* translation_domain, GError** mcerror)
{
    gboolean ok = TRUE;
    ...
    terminal_group = g_option_group_new("terminal", _("Terminal options"), _("Terminal options"), nullptr, nullptr);
    ...
    if (!g_option_context_parse(context, argc, argv, mcerror))
    {
        ...
        {
            gchar *help_str = g_option_context_get_help(context, TRUE, nullptr);

            if (str_isutf8(_system_codepage))
                mc_replace_error(mcerror, (*mcerror)->code, "%s\n\n%s\n", (*mcerror)->message, help_str);
            else
            {
                gchar *full_help_str = mc_args__convert_help_to_syscharset(_system_codepage, (*mcerror)->message, help_str);
...
gboolean Args::mc_args_show_info()
{
...
gboolean Args::mc_setup_by_args(int argc, char** argv, GError** mcerror)
{
    char *tmp;
...
void Args::mcedit_arg_free(mcedit_arg_t* arg)
{
    vfs_path_free(arg->file_vpath);
    g_free(arg);
}

GList* Args::parse_mcedit_arguments(int argc, char** argv)
{
    GList *flist = nullptr;
    int i;
...
Args::mcedit_arg_t* Args::mcedit_arg_new (const char *file_name, long line_number)
{
    return mcedit_arg_vpath_new(vfs_path_from_str(file_name), line_number);
}

Args::mcedit_arg_t* Args::mcedit_arg_vpath_new(vfs_path_t* file_vpath, long line_number)
{
    mcedit_arg_t *arg = g_new (mcedit_arg_t, 1);
...
gboolean Args::parse_mc_v_argument(const char*, const char*, void*, GError** mcerror)
{
    if (mcerror != nullptr && *mcerror != nullptr)
        return FALSE;
...
gboolean Args::parse_mc_e_argument(const char*, const char*, void*, GError ** mcerror)
{
    if (mcerror != nullptr && *mcerror != nullptr)
        return FALSE;
...
char* Args::mc_args__convert_help_to_syscharset(const char* charset, const char* error_message_str, const char* help_str)
{
...
void Args::mc_args_add_extended_info_to_help()
{
...
char* Args::mc_args_add_usage_info()
{
    gchar *s;
...
GOptionGroup* Args::mc_args_new_color_group()
{
/* *INDENT-OFF* */
...
/* *INDENT-ON* */
    ...
    return g_option_group_new("color", mc_args__loc__colors_string, _("Color options"), nullptr, nullptr);
}

void Args::mc_args_clean_temp_help_strings()
{
