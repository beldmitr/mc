...
open_include_file (const char *filename)
{
    ...
    g_free (error_file_name);
    error_file_name = g_build_filename (mc_global.sysconfig_dir.c_str(), "syntax", filename, (char *) NULL);
    f = fopen (error_file_name, "r");
    if (f != NULL)
        return f;
    ...
    g_free (error_file_name);
    error_file_name =
        g_build_filename (mc_global.share_data_dir.c_str(), "syntax", filename, (char *) NULL);
                       ...
                       const char *editor_file, const char *first_line, const char *type)
{
    ...
    if (f == NULL)
    {
        lib_file = g_build_filename (mc_global.share_data_dir.c_str(), "syntax", "Syntax", (char *) NULL);
        f = fopen (lib_file, "r");
        g_free (lib_file);
        if (f == NULL)
            return -1;
