...
gboolean UserMenu::user_menu_cmd (const WEdit * edit_widget, const char *menu_file, int selected_entry)
{
    char *p;
    char *data, **entries;
...
char* UserMenu::expand_format (const WEdit * edit_widget, char c, gboolean do_quote)
{
    WPanel *panel = NULL;
...
int UserMenu::check_format_view (const char *p)
{
    const char *q = p;
...
int UserMenu::check_format_var (const char *p, char **v)
{
    *v = NULL;
...
int UserMenu::check_format_cd (const char *p)
{
    return (strncmp (p, "cd", 2)) != 0 ? 0 : 3;
}

gboolean UserMenu::menu_file_own (char *path)
{
    struct stat st;
...
void UserMenu::execute_menu_command (const WEdit * edit_widget, const char *commands, gboolean show_prompt)
{
    FILE *cmd_file;
    int cmd_file_fd;
    ...
    for (col = 0; *commands != '\0'; commands++)
    {
        ...
        if (parameter != NULL)
        {
            if (*commands == '}')
            {
                *parameter = '\0';
                parameter =
                        input_dialog (_("Parameter"), lc_prompt, History::MC_HISTORY_FM_MENU_EXEC_PARAM, "",
...
char* UserMenu::test_line (const WEdit * edit_widget, char *p, gboolean * result)
{
    char Operator;
...
void UserMenu::debug_out (char *start, char *end, gboolean condition)
{
...
gboolean UserMenu::test_type (WPanel * panel, char *arg)
{
...
char* UserMenu::extract_arg (char *p, char *arg, int size)
{
...
char* UserMenu::check_patterns (char *p)
{
...
char* UserMenu::strip_ext (char *ss)
{
    char *s = ss;
    char *e = NULL;
