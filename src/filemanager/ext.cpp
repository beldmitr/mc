...
exec_expand_format (char symbol, gboolean is_result_quoted)
{
    ...
    text = UserMenu::ExpandFormat(NULL, symbol, TRUE);
    ...
    for (; *lc_data != '\0' && *lc_data != '\n'; lc_data++)
    {
        ...
        else if (expand_prefix_found)
        {
            ...
            else
            {
                ...
                int i = UserMenu::CheckFormatView(lc_data);
                if (i != 0)
                {
                    lc_data += i - 1;
                    run_view = TRUE;
                ...
                {
                    i = UserMenu::CheckFormatCD(lc_data);
                    if (i > 0)
                    {
                        is_cd = TRUE;
                    ...
                    {
                        i = UserMenu::CheckFormatVar(lc_data, &v);
                        if (i > 0 && v != NULL)
                        {
                        ...
                        else
                        {
                            ...
                            if (*lc_data != 'f')
                                text = UserMenu::ExpandFormat(NULL, *lc_data, !is_cd);
                            else
                            {
                                text = exec_get_file_name (filename_vpath);
                                if (text == NULL)
