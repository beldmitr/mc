...
#include "helpers/include/includes.hpp"
...
smbfs_auth_remove (const char *host, const char *share)
{
    ...
    if (!list)
        return;
    auth = static_cast<smb_authinfo *>(list->data);
...
static void
smbfs_browsing_helper (const char *name, uint32_t type, const char *comment, void *state)
{
    const char *typestr = "";
...
static void
smbfs_srv_browsing_helper (const char *name, uint32_t m, const char *comment, void *state)
{
    dir_entry *new_entry = smbfs_new_dir_entry (name);
