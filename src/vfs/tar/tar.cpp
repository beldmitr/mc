...
int Tar::tar_fh_open(struct vfs_class* me, vfs_file_handler_t* fh, int flags, mode_t mode)
{
    (void) fh;
    (void) mode;
...
ssize_t Tar::tar_read(void* fh, char* buffer, size_t count)
{
    ...
    int fd = reinterpret_cast<tar_super_t*>(VFS_FILE_HANDLER_SUPER (fh))->fd;
    ssize_t res;
...
int Tar::tar_super_same(const vfs_path_element_t* vpath_element, struct vfs_s_super* parc, const vfs_path_t* vpath, void* cookie)
{
    auto *archive_stat = static_cast<struct stat*>(cookie); /* stat of main archive */

    if (strcmp (parc->name, vfs_path_as_str (vpath)) != 0)
        return 0;

    /* Has the cached archive been changed on the disk? */
    if (parc && reinterpret_cast<tar_super_t *>(parc)->st.st_mtime < archive_stat->st_mtime)
    {
        /* Yes, reload! */
...
void* Tar::tar_super_check(const vfs_path_t* vpath)
{
...
int Tar::tar_open_archive(struct vfs_s_super* archive, const vfs_path_t* vpath, const vfs_path_element_t* vpath_element)
{
...
Tar::ReadStatus Tar::tar_read_header(struct vfs_class* me, struct vfs_s_super* archive, int tard, size_t* h_size)
{
    auto* arch = reinterpret_cast<tar_super_t*>(archive);
...
void Tar::tar_fill_stat(struct vfs_s_super* archive, struct stat* st, union block* header, size_t h_size)
{
    auto *arch = reinterpret_cast<tar_super_t*>(archive);
...
size_t Tar::tar_decode_header (union block *header, tar_super_t * arch)
{
    size_t size;
...
Tar::ReadStatus Tar::tar_checksum(const union block* header)
{
    long recsum;
    long signed_sum = 0;
    long sum = 0;
...
void Tar::tar_skip_n_records (struct vfs_s_super *archive, int tard, size_t n)
{
    (void) archive;
...
union Tar::block* Tar::tar_get_next_block(struct vfs_s_super* archive, int tard)
{
    int n;
...
int Tar::tar_open_archive_int (struct vfs_class* me, const vfs_path_t* vpath, struct vfs_s_super* archive)
{
    int result, type;
    tar_super_t *arch;
    mode_t mode;
    ...
    arch = reinterpret_cast<tar_super_t*>(archive);
...
void Tar::tar_free_archive (struct vfs_class* me, struct vfs_s_super* archive)
{
    auto *arch = reinterpret_cast<tar_super_t*>(archive);
...
struct vfs_s_super* Tar::tar_new_archive (struct vfs_class* me)
{
    tar_super_t *arch;
...
long Tar::tar_from_oct(int digs, const char* where)
{
    long value;
