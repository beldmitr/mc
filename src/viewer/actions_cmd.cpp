...
{
    // FIXME DB: reinterpret_cast isn't good, let's change the function param
    // Or better idea, WView should be inherited from Widget
...
cb_ret_t ActionsCmd::mcview_dialog_callback(Widget* w, Widget* sender, widget_msg_t msg, int parm, void* data)
{
    WDialog *h = DIALOG(w);
    WView *view;
...
gboolean ActionsCmd::mcview_ok_to_quit(WView* view)
{
    int r;
...
void ActionsCmd::mcview_resize(WView* view)
{
...
cb_ret_t ActionsCmd::mcview_handle_key(WView* view, int key)
{
#ifdef HAVE_CHARSET
    ...
    long command = mcview_lookup_key (view, key);
...
long ActionsCmd::mcview_lookup_key(WView* view, int key)
{
    if (view->mode_flags.hex)
...
cb_ret_t ActionsCmd::mcview_execute_cmd(WView* view, long command)
{
...
void ActionsCmd::mcview_load_file_from_history(WView* view)
{
    int action;
    char *filename = FileHistory::show_file_history(CONST_WIDGET (view), &action);
    ...
    if (filename != nullptr && (action == CK_View || action == CK_Enter))
    {
        ...
        McViewer::mcview_load(view, nullptr, filename, 0, 0, 0);
...
void ActionsCmd::mcview_load_next_prev(WView* view, int direction)
{
    dir_list *dir;
    int *dir_idx;
    vfs_path_t *vfile;
    ...
    if (regex_command_for (view, vfile, "View", &ext_script) == 0)
        McViewer::mcview_load (view, nullptr, vfs_path_as_str (vfile), 0, 0, 0);
    vfs_path_free (vfile);
    view->dir = dir;
...
void ActionsCmd::mcview_scan_for_file(WView* view, int direction)
{
    int i;
...
void ActionsCmd::mcview_load_next_prev_init(WView* view)
{
...
void ActionsCmd::mcview_continue_search_cmd(WView* view)
{
...
void ActionsCmd::mcview_search(WView* view, gboolean start_search)
{
...
void ActionsCmd::mcview_remove_ext_script(WView* view)
{
