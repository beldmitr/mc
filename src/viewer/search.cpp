...
{
    WView* view = (static_cast<const mcview_search_status_msg_t*>(user_data))->view;
...
mc_search_cbret_t Search::mcview_search_update_cmd_callback(const void* user_data, std::size_t char_offset)
{
...
void Search::mcview_do_search(WView* view, off_t want_search_start)
{
    ...
    status_msg_init (STATUS_MSG (&vsm), _("Search"), 1.0, simple_status_msg_init_cb, mcview_search_status_update_cb, nullptr);
...
void Search::mcview_search_show_result(WView* view, std::size_t match_len)
{
    int nroff_len;
