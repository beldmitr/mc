...
Setup::panels_options_t Setup::panels_options;

const Setup::list_formats_t Setup::list_formats[] = {
    {"full", list_full},
    {"brief", list_brief},
    {"long", list_long},
    {"user", list_user},
...
const Setup::panel_types_t Setup::panel_types[] = {
...
const Setup::layout_int_options_t Setup::layout_int_options[] = {
...
const Setup::layout_bool_options_t Setup::layout_bool_options[] = {
...
const struct Setup::bool_options_t Setup::bool_options[] = {
    {"verbose", &verbose},
...
const Setup::str_options_t Setup::str_options[] = {
#ifdef USE_INTERNAL_EDIT
...
const Setup::panels_ini_options_t Setup::panels_ini_options[] = {
...
void Setup::load_setup()
{
    ...
    if ((autodetect_codeset[0] != '\0') && (strcmp(autodetect_codeset, "off") != 0))  //FIXME DB: strcmp
...
gboolean Setup::save_setup(gboolean save_options, gboolean save_panel_options)
{
    gboolean ret = TRUE;
...
void Setup::setup_save_config_show_error(const char *filename, GError **mcerror)
{
...
#ifdef ENABLE_VFS_FTP
char* Setup::load_anon_passwd()
{
    char *buffer = mc_config_get_string(mc_global.main_config, CONFIG_MISC_SECTION, "ftpfs_password", "");
...
void Setup::load_keymap_defs(gboolean load_from_file)
{
    /*
...
void Setup::panel_load_setup(WPanel *panel, const char *section)
{
    size_t i;
...
void Setup::panel_save_setup(WPanel *panel, const char *section)
{
...
char* Setup::load_setup_get_full_config_name(const char *subdir, const char *config_file_name)
{
    // TODO DB: next comment ?
    /*
...
const char* Setup::setup__is_cfg_group_must_panel_config(const char *grp)
{
...
void Setup::setup__move_panels_config_into_separate_file(const char *profile)
{
    mc_config_t *tmp_cfg;
...
void Setup::load_setup_init_config_from_file(mc_config_t **config, const char *fname, gboolean read_only)
{
    // FIXME DB: next comment ?
    /*
       TODO: IMHO, in future, this function shall be placed in mcconfig module.
     */
    if (exist_file(fname)) {
...
void Setup::load_config()
{
    ...
    if (kt != nullptr && kt[0] != '\0')
        old_esc_mode_timeout = atoi(kt);      // TODO DB: investigate warning
}

panel_view_mode_t Setup::setup__load_panel_state(const char *section)
{
    char *buffer;
    size_t i;
...
void Setup::load_keys_from_section(const char *terminal, mc_config_t *cfg)
{
    char *section_name;
...
void Setup::load_keymap_from_section(const char *section_name, GArray *keymap, mc_config_t *cfg)
{
    gchar **profile_keys, **keys;
...
mc_config_t* Setup::load_setup_get_keymap_profile_config(gboolean load_from_file)
{
    // TODO DB: next comment
    /*
       TODO: IMHO, in future, this function shall be placed in mcconfig module.
    ...
    /* 3) --keymap=<keymap> */
    fname = load_setup_get_full_config_name(NULL, Args::mc_args__keymap_file);
    if (fname != nullptr && strcmp(fname, sysconfig_keymap) != 0 && strcmp(fname, share_keymap) != 0) { // TODO DB: strcmp
...
void Setup::panel_save_type(const char *section, panel_view_mode_t type)
{
    size_t i;
...
void Setup::panels_load_options()
{
...
void Setup::panels_save_options()
{
    size_t i;
